#!/usr/bin/env python3
"""
Dioptra: Monocular Metric 3D Depth Estimation.
...
Quick Commands:
  ...
  python3 dioptra_mac.py --smoke
  python3 dioptra_mac.py --test
  python3 dioptra_mac.py --predict sample.jpg
  python3 dioptra_mac.py --predict sample.jpg --export-ply scene.ply
"""
...
def print_hardware_specs() -> None:
    """Print detailed Apple Silicon specs and Dioptra memory budgeting."""
    ...
    print("-" * 70)
    print("  DIOPTRA MEMORY BUDGET ANALYSIS (8GB UNIFIED MEMORY)")
    print("-" * 70)
    ...
    print("  ----------------------------------------------------------------------")
    print(f"  Verdict: Dioptra v1.1 is fully verified to run comfortably on this {chip} with {total_ram_gb:.0f}GB RAM.")
    print("=" * 70)
...
def check_torch_installed() -> bool:
    ...
    try:
        ...
        print("         python3 dioptra_mac.py --smoke")
        print("!" * 70 + "\n")
        return False
...
def main():
    parser = argparse.ArgumentParser(
        description="Dioptra v1.1 — Apple Silicon Edition (MacBook Air M3 / Pro)",
...
Examples:
  ...
  python3 dioptra_mac.py --smoke
  python3 dioptra_mac.py --test
  python3 dioptra_mac.py --predict room.jpg
  python3 dioptra_mac.py --predict room.jpg --export-ply room_3d.ply
  python3 dioptra_mac.py --train /path/to/tartanair --batch-size 2
        """,
    )
    ...
    import dioptra_v1_patched as tess
    ...
    if args.smoke:
        ...
        model = tess.Dioptra(model_cfg).to(device)
        model.eval()
    ...
    if args.predict:
        ...
        cfg = tess.DioptraConfig()
        model = tess.Dioptra(cfg.model).to(device)
