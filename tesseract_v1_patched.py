"""
═══════════════════════════════════════════════════════════════════════════════
  TESSERACT v1.1 (v17 Refinement) — Single-File Implementation
  3D Spatial Foundation Model for Monocular Depth Estimation
═══════════════════════════════════════════════════════════════════════════════
...
class Continuous3DAttention(nn.Module):
    ...
    def forward(
        ...
        attn_mask = None
        if query_rays is not None and key_points is not None and irer_gate > 0.0:
            attn_mask = self._compute_irer(query_rays, key_points, B, N, irer_gate)
    ...
    def _compute_irer(self, query_rays: Tensor, key_points: Tensor,
                      B: int, N: int, irer_gate: float = 1.0) -> Tensor:
        """Compute IRER bias with geometric/appearance head split.

        Only the first num_geometric_heads heads receive IRER bias;
        the remaining appearance heads get zero bias.

        Args:
            query_rays: (B, N, 3, 3) — 3 rays per token.
            key_points: (B, N, 3) — 3D point per token.
...
class TransformerBlock(nn.Module):
    ...
    def forward(
        ...
        x = x + self.drop_path1(self.ls1(
            self.attn(self.norm1(x), query_rays, key_points, irer_gate)
        ))
        # MLP residual
        ...
        for i, block in enumerate(self.blocks):
            if self.gradient_checkpointing and self.training:
                ...
                x = torch.utils.checkpoint.checkpoint(
                    block, x, eff_rays, eff_keys, eff_gate,
                    use_reentrant=False,
                )
            else:
                x = block(x, eff_rays, eff_keys, eff_gate)
