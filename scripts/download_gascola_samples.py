"""
Download a portion of an unseen TartanAir environment (Gascola Easy P001, 30 frames).
Uses HTTP range requests to extract individual files directly from HuggingFace
without downloading the 7 GB zip archive.
"""
...
def download_gascola_subset(num_frames=30, out_dir="test_samples/unseen_gascola_p001"):
    os.makedirs(os.path.join(out_dir, "image_left"), exist_ok=True)
    os.makedirs(os.path.join(out_dir, "depth_left"), exist_ok=True)

    img_url = "https://huggingface.co/datasets/theairlabcmu/tartanair/resolve/main/gascola/Easy/image_left.zip"
    depth_url = "https://huggingface.co/datasets/theairlabcmu/tartanair/resolve/main/gascola/Easy/depth_left.zip"

    print(f"[1/4] Connecting to Gascola image archive...")
    zf_img = zipfile.ZipFile(HttpRangeFile(img_url))
    print(f"[2/4] Connecting to Gascola depth archive...")
    ...
    print(f"[3/4] Downloading {len(img_names)} consecutive RGB and Depth pairs from Gascola/Easy/P001...")
...
if __name__ == "__main__":
    download_gascola_subset(num_frames=30)
